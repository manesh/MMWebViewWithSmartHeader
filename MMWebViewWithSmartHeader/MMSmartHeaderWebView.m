    ...
    if (newValue == YES) {
        ...
        if (numberOfPointsOfHeaderVisible < self.headerView.frame.size.height) {
            [UIView animateWithDuration:0.2 animations:^ {
                ...
                headerFrame.origin.y = MAX(-self.headerView.frame.size.height, self.webView.scrollView.contentOffset.y);
                self.headerView.frame = headerFrame;
                self.webView.scrollView.scrollIndicatorInsets = UIEdgeInsetsMake(MAX(self.headerView.frame.size.height, -self.webView.scrollView.contentOffset.y), 0, 0, 0);
            }];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pinHeader) {
        ...
        self.headerView.frame = headerFrame;
        self.webView.scrollView.scrollIndicatorInsets = UIEdgeInsetsMake(MAX(self.headerView.frame.size.height, -scrollView.contentOffset.y), 0, 0, 0);
    }
